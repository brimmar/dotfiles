...
try:
    ...
    with open(oauth_file) as f:
        data = json.load(f)
    token_data = data.get('token', {}) if 'token' in data else data
    payload = json.dumps({
        'token': {
    ...
    # Write fallback token file for immediate, headless, and SSH usage
    ...
    # Register in GNOME Keyring / Secret Service if available and unlocked
    try:
        import secretstorage
        bus = secretstorage.dbus_init()
        col = secretstorage.get_default_collection(bus)
        if not col.is_locked():
            col.create_item(\\"Password for 'antigravity' on 'gemini'\\", {'service': 'gemini', 'username': 'antigravity', 'xdg:schema': 'org.freedesktop.Secret.Generic'}, payload.encode('utf-8'), replace=True)
    except Exception:
        pass
except Exception:
    pass
